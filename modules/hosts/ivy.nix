...
{ config, inputs, ... }: {
    flake.modules.nixos."hosts/ivy" = { lib, ... }: {
        ...
        # Static address: ivy is the network's DNS + DHCP server, so it
        # cannot get its own address from a lease. Ivy resolves through its
        # own AdGuard (the static profile provides no DNS of its own).
